...
SERVICE_LOG_PATH = os.environ.get(
    "SERVICE_LOG_PATH", "data/service_logs/match_service.log"
)
...
def write_service_log(request_id, client_country, entity_value, status_code,
                     processing_time_ms, scenario="normal"):
    """
    Structured JSON Lines logging (one independent JSON object per line).

    Output file: data/service_logs/match_service.log  (override with $SERVICE_LOG_PATH)

    Fields (see Phase 1 spec, section 7.2 / 7.3):
    ...
    request_id / client_country / scenario originate from the X-Request-ID,
    X-Client-Country and X-Scenario headers that Nginx forwards to us.
    """
    record = {
    ...
    # A single append-mode write of one short line is atomic enough on Linux,
    # but uvicorn serves sync endpoints from a thread pool, so we still guard
    # the file handle with a lock to avoid interleaved lines.
    with _log_lock:
